...
const DisplayArea = ({ expression, result, isError = false }) => {
  ...
  return (
    <div className="grid grid-cols-1 grid-rows-[50%_50%] items-center border border-white rounded-md col-span-4 bg-white/80 text-black text-xl md:text-3xl lg:text-5xl">
      ...
      <div
        className={`text-right me-2 md:me-8 ${resultColor} align-middle overflow-x-auto whitespace-nowrap`}
      >
...
const InputButton = ({ char, onClick, color }) => {
  return (
    <button
      className={`col-span-1 border rounded-3xl border-zinc-500/90 ${BUTTON_COLORS[color]}`}
...
function Calculator() {
    ...
    (num) => {
      if (expressionAndResult.needsReset) {
        ...
        setExpressionAndResult({
          expression: [...expressionAndResult.expression, num],
          result: [],
          needsReset: false,
        });
      }
  ...
  const handleOperationInput = useCallback(
    (op) => {
      if (expressionAndResult.needsReset) {
        resetExpression([
          previousNonErrorResult !== null
            ? previousNonErrorResult
            : expressionAndResult.result[1],
          op,
        ]);
      } else {
        setExpressionAndResult({
          expression: [...expressionAndResult.expression, op],
          result: [],
          needsReset: false,
        });
      }
    ...
    [
      ...
      expressionAndResult.result,
      expressionAndResult.needsReset,
      previousNonErrorResult,
    ],
  );
  ...
  const backspace = useCallback(() => {
    setExpressionAndResult({
      expression: expressionAndResult.expression.slice(0, -1),
      result: [],
      needsReset: false,
    });
  }, [expressionAndResult.expression]);
  ...
  const computeExpression = useCallback(() => {
    ...
    try {
      ...
      setExpressionAndResult({
        expression: [...expressionAndResult.expression],
    ...
    const resultDisplay = resultOfComputation.match(/[A-Za-z]+/g)
      ? `\\text{${resultOfComputation}}`
      : resultOfComputation;
    setExpressionAndResult({
      expression: [...expressionAndResult.expression],
  ...
  return (
    <div className="font-sans min-h-svh grid grid-cols-[auto_minmax(300px,_90%)_auto] grid-rows-[auto_minmax(400px,_90%)_auto] text-3xl bg-black">
      ...
      <div className="col-span-1 row-span-1 grid grid-cols-4 md:grid-rows-[2fr_1fr_1fr_1fr_1fr_1fr] grid-rows-[80px_1fr_1fr_1fr_1fr_1fr] gap-6 p-6 bg-slate-900 border border-white/80 text-white">
        <DisplayArea
          expression={
            expressionAndResult.expression.length > 0
              ? parseExpression(expressionAndResult.expression)
              : ""
          }
          result={
            expressionAndResult.result.length > 0
              ? parseExpression(expressionAndResult.result)
              : ""
          }
      ...
      <div className="grid grid-cols-2 grid-rows-1 col-span-1 row-span-1 align-middle mt-4 md:mt-10 text-center">
        <IconLink
          ...
          link="https://github.com/ProjectGrantwood"
        />
        <IconLink
          imagePath={linkedinIcon}
          link="https://www.linkedin.com/in/jptphiladelphia/"
        />
      </div>
      <div className="col-span-1 row-span-1"></div>
    </div>
